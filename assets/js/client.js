...
connection.onmessage=function(msg){
  var data=JSON.parse(msg.data);
    ...
    switch (data.type){
        ...
        break;
        case "offer":
            call_status.innerHTML='<div class="calling-status-wrap card black white-text"> <div class="user-image"> <img src="assets/images/test.jpg" class="caller-image circle" alt=""> </div> <div class="user-name"> '+data.name+' </div> <div class="user-calling-status"> Calling.... </div> <div class="calling-action"> <div class="call-accept"> <i class="material-icons green darken-2 white-text audio-icon">call</i> </div> <div class="call-reject"> <i class="material-icons red darken-3 white-text close-icon">close</i> </div> </div> </div>';
...
var cal_btn=document.querySelector("#call-btn");
...
function successProccess(stream){
    ...
    myConn= new webkitRTCPeerConnection(configuration,{optional:[{
...
function loginProcess(success){
    if(success===false){
        ...
        navigator.mediaDevices.getDisplayMedia({
            video:true,
            audio:true
        }).then(successProccess,errorProcess);
...
msgSendBtn.addEventListener("click",function(){
    ...
    chatArea.innerHTML += "<div class='right-align' style='display:flex;align-items:center;align-self:flex-end;'><div>"+msgVal+"</div>:<div style='font-weight:600;margin:0 5px;'> "+name+"</div><img src='assets/images/test.jpg' style='height:40px;width:40px;' class='caller-image circle'></div><br/>";
